  ...
  return (
    <div className="space-y-16 lg:space-y-24 pb-12">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="mx-auto max-w-3xl text-center space-y-6">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white font-poppins leading-tight">
              {shopName}{locationStr ? <span className="text-amber-400"> — Barbershop in {locationStr}</span> : null}
            </h1>
      ...
      <section className="bg-zinc-900/50 py-16 border-y border-zinc-800/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
            ...
            <p className="text-zinc-400 text-sm">Our team is being assembled. Check back soon!</p>
          </div>
          )}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review) => (
            <Card key={review.id} className="bg-zinc-900/90 border-zinc-800">
              <CardContent className="pt-6 space-y-3">
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${i < review.rating ? 'text-amber-400 fill-amber-400' : 'text-zinc-700'}`}
                    />
                  ))}
                </div>
                ...
                <p className="text-xs font-semibold text-zinc-400">— {review.authorName}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="bg-zinc-900 border-zinc-800 p-6 space-y-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Our Location</h3>
                <p className="text-sm text-zinc-400 mt-1">{fullAddress}</p>
                <a
                  href={mapsLinkUrl}
                  target="_blank"
            ...
            {shopPhoneDigits && (
              <div className="border-t border-zinc-800 pt-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {business?.email && (
                    <a href={`mailto:${business.email}`} className="text-sm text-amber-400 hover:underline">
                      ✉️ {business.email}
                    </a>
                  )}
                </div>
          ...
          {/* Google Maps Embed */}
          <Card className="bg-zinc-900 border-zinc-800 relative overflow-hidden min-h-[320px] p-0">
            <iframe
              src={mapsEmbedUrl}
              width="100%"
              height="100%"
